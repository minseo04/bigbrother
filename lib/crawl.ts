...
export async function crawlDue(owner:string,entities:Entity[],batch=BATCH){const db=database();const due=await db.prepare("SELECT id,name,feed_url FROM entities WHERE owner_id=? AND followed=1 ORDER BY last_crawled ASC,rowid ASC LIMIT ?").bind(owner,Math.max(1,Math.min(BATCH,batch))).all<{id:string;name:string;feed_url:string}>();const now=new Date().toISOString();if(due.results.length)await db.batch(due.results.map(e=>db.prepare("UPDATE entities SET last_crawled=? WHERE owner_id=? AND id=?").bind(now,owner,e.id)));return Promise.all(due.results.map(row=>{const entity=entities.find(e=>e.id===row.id);if(!entity)throw new Error("Entity not found.");const feed=entityFeed({...entity,feedUrl:row.feed_url});return fetchFeed({id:"entity:"+row.id,name:row.name,url:new URL(feed).origin,feed},entities);}));}
